...
function compute_smoke_scc(m::Model = get_smoke_model(); 
    ...
    year === nothing ? error("Must specify an emission year. Try `compute_smoke_scc(m, year=2020)`.") : nothing
    ...
    if n==0
        ...
        isnothing(discount_rates) ? error("To run the Monte Carlo compute_smoke_scc function (n != 0), please use the `discount_rates` argument.") : nothing
        ...
        return _compute_smoke_scc_mcs(mm, 
                                n,
                                year = year,
                                last_year = last_year,
                                discount_rates = discount_rates,
    ...
    if discount_rates!==nothing
        ...
        for dr in discount_rates
            ...
            elseif dr.ew==:gdp_country
                ...
                smoke_marginal_damages = mm[:Smoke, :smoke_costs] .* scc_gas_molecular_conversions[gas] # country
                ...
                pc_gdp_for_smoke = mm.base[:PerCapitaGDP, :pc_gdp]
                n_regions_for_smoke = size(pc_gdp_for_smoke, 2)
                smoke_scc_in_utils = sum(
                    smoke_marginal_damages[i,r] / pc_gdp_for_smoke[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                    for (i,t) in enumerate(_model_years), r in 1:n_regions_for_smoke if year<=t<=last_year
                )
                ...
                total_utils = 
                    ...
                    (mm.base[:DamageAggregator, :include_smoke]       ? smoke_scc_in_utils : 0.) +
...
function smoke_post_trial_func(mcs::SimulationInstance, trialnum::Int, ntimesteps::Int, tup)
    ...
    if options.compute_sectoral_values
        ...
        smoke_mds = post_trial_mm[:DamageAggregator, :smoke_damage]
        ...
        if options.compute_domestic_values
            ...
            smoke_mds_domestic = post_trial_mm[:DamageAggregator, :smoke_damage_domestic]
        end
    end
    ...
    for dr in discount_rates
        ...
        if isnothing(dr.ew)
            ...
            if options.compute_sectoral_values
                ...
                scc_values[(region=:globe, sector=:smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                ...
                if options.certainty_equivalent
                    ...
                    intermediate_ce_scc_values[(region=:globe, sector=:smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = intermediate_ce_scc
                ...
                if options.compute_domestic_values
                    ...
                    scc_values[(region=:domestic, sector= :smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                    ...
                    if options.certainty_equivalent
                        ...
                        intermediate_ce_scc_values[(region=:domestic, sector= :smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = intermediate_ce_scc
        ...
        elseif dr.ew==:gdp_region || dr.ew==:gdp_country
            if dr.ew==:gdp_region
                ...
                smoke_marginal_damages = post_trial_mm[:DamageAggregator, :damage_smoke_regions] # fund regions 
            ...
            elseif dr.ew==:gdp_country
                ...
                smoke_marginal_damages = post_trial_mm[:Smoke, :smoke_costs]
            ...
            smoke_scc_in_utils = sum(
                smoke_marginal_damages[i,r] / pc_gdp[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                for (i,t) in enumerate(_model_years), r in 1:n_regions if year<=t<=last_year
            )
            ...
            total_utils =
                ...
                (base[:DamageAggregator, :include_smoke]       ? smoke_scc_in_utils : 0.) +
            ...
            if options.compute_sectoral_values
                ...
                scc = base[pc_gdp_component_name, :pc_gdp][year_index,normalization_region_index]^dr.eta * smoke_scc_in_utils
                scc_values[(region=:globe, sector=:smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
                ...
                if options.certainty_equivalent
                    ...
                    intermediate_ce_scc_values[(region=:globe, sector=:smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = smoke_scc_in_utils
        ...
        elseif dr.ew==:consumption_region || dr.ew==:consumption_country
            ...
            smoke_scc_in_utils = sum(
                smoke_marginal_damages[i,r] / pc_consumption[i,r]^dr.eta * 1/(1+dr.prtp)^(t-year)
                for (i,t) in enumerate(_model_years), r in 1:n_regions if year<=t<=last_year
            )
            ...
            if options.compute_sectoral_values
                ...
                scc = base[net_cpc_component_name, :net_cpc][year_index,normalization_region_index]^dr.eta * smoke_scc_in_utils
                scc_values[(region=:globe, sector=:smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta, ew=dr.ew, ew_norm_region=dr.ew_norm_region)][trialnum] = scc
...
function _compute_smoke_scc_mcs(mm::MarginalModel, 
    ...
    sectors = compute_sectoral_values ? [:total,  :cromar_mortality, :agriculture, :energy, :smoke, :slr] : [:total]
    ...
    if compute_disaggregated_values
        ...
        mkpath(joinpath(top_path, "damages_smoke"))		
    ...
    sim_results = run(mcs, 
                        models, 
                        n, 
                        post_trial_func = smoke_post_trial_func,
