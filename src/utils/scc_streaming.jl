...
function _smoke_stream_disagg_damages(m::Mimi.Model, output_dir::String, trialnum::Int, streams::Dict)	
    ...
    smoke_damages = getdataframe(m, :DamageAggregator, :damage_smoke) |> 	
                        @filter(_.time > 2019) |> 	
                        @mutate(damage_smoke = _.damage_smoke) |>
                        @rename(:damage_smoke => :damages, :smoke_countries => :country) |>	
                        DataFrame |>	
                        i -> insertcols!(i, 1, :trialnum => trialnum)	
    ...
    for country in unique(smoke_damages.country)	
        filename = joinpath("$output_dir/results/disaggregated_values/damages_smoke/$(country).csv")	
        trial_df = smoke_damages |> @filter(_.country == country) |> @select(:trialnum, :time, :damages) |> DataFrame	
        if haskey(streams, filename)	
            write(streams[filename], trial_df)	
        else	
            streams[filename] = savestreaming(filename, trial_df)	
...
function _smoke_stream_disagg_md(m_base::Mimi.Model, m_modified::Mimi.Model, ciam_base::Union{Nothing, Mimi.ModelInstance}, md_ciam::Union{Nothing, Array}, 	
    ...
    md_smoke       = (view(m_modified[:DamageAggregator, :damage_smoke], _damages_idxs,:) .- view(m_base[:DamageAggregator, :damage_smoke], _damages_idxs,:)) .* gas_units_multiplier	
    ...
    md_country_df = DataFrame(md_cromar_mortality .+ md_energy .+ md_smoke .+ md_ciam_all_countries, dim_keys(m_base, :country)) |>	
                    i -> insertcols!(i, 1, :time => _damages_years) |>	
                    i -> stack(i, Not(:time)) |>	
